namespace Boardgames.Utilities
{
    public class Constance
    {
        //BoardGame
        public const int MaxNameLenght = 20;
        public const int MinNameLenght = 10;

        public const int MaxRating = 10;
        public const int MinRating = 1;

        public const int MaxYearPublished = 2023;
        public const int MinYearPublished = 2018;



        //Seller
        public const int MaxSellerName = 20;

        public const int MaxAddressLenght = 30;

        public const int MaxFirstNameCreator = 7;
        public const int MinFirstNameCreator = 2;


        public const int MaxLastNameCreator = 7;
        public const int MinLastNameCreator = 2;

    }
}
